...
class Iso:
    ...
    def queryCoef(self, cible, va, granu=None, solu=None, org='efficace', logger=logging.getLogger()):
        ...
        if granu != None and granu not in self.dataDic[2].values():
            msg = "La granu %s n'existe pas. Les possibilités sont : " % granu + str(self.dataDic[2].values())
            logger.error(msg)
            raise KeyError(msg)

        if solu != None and solu not in self.dataDic[3].values():
        ...
        if granu == None and solu == 'V':
            granu = 'none'
        elif granu == None and solu != 'V':
            granu = self.defGranu
...
def dicDoseBuild(pcFileCoef):
    ...
    with open(pcFileCoef, 'r') as pfi:
        for line in pfi:
            ...
            elif len(line) == 3 and line[-1] == '-999':
